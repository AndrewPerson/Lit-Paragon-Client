...
var _Extensions = class {
  ...
  static async HandleCommand(e7) {
    let command = e7.data.command;
    let data = e7.data.data;
  ...
  static AddListeners() {
    ...
    window.addEventListener("message", async (e7) => {
      ...
      if (!this.extensionOrigins.includes(origin))
        return;
      e7.source?.postMessage(await this.HandleCommand(e7), {
        targetOrigin: origin
      });
    });
  }
...
async function Token(code2) {
  let tokenResponse = await fetch("https://sbhs-random-data.profsmart.repl.co/auth", {
    method: "POST",
    body: code2
  });
  ...
  if (!tokenResponse)
    return false;
  let resourceCache = await caches.open("User Resources");
