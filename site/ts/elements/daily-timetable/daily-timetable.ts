...
export class StudentDailyTimetable extends Page {
    ...
    @state()
    private dailyTimetable: DailyTimetable;
    ...
    constructor() {
        ...
        this.addEventListener("countdown-finished", (_ => {
            const currentBellIndex = getCurrentBellIndex(this.dailyTimetable, new Date());

            if (currentBellIndex === undefined) {
                getNewDailyTimetable(this.dailyTimetable).then(newDailyTimetable => {
    ...
    renderPage() {
        const nextBellIndex = getNextBellIndex(this.dailyTimetable, new Date());
        const nextBell = nextBellIndex === undefined ? undefined : this.dailyTimetable.timetable[nextBellIndex];
        const nextVisibleBell = nextBellIndex === undefined ? undefined : this.dailyTimetable.timetable.slice(nextBellIndex).find(bell => bell.shouldDisplay);

        return html`
            <info-popup style="${this.dailyTimetable.autoGenerated ? "" : "display: none"}">
            ...
            ${
                ...
                html`<dt-countdown name="${nextBell.name}" .time="${timeToDate(nextBell.startTime, new Date(this.dailyTimetable.date))}"></dt-countdown>`
            }

            <div class="periods">
                ${map(
                    this.dailyTimetable.timetable.filter(bell => bell != null && bell.shouldDisplay) as (Bell | Period)[],
                    (bell: Bell | Period) =>
                        bell.type === "bell" ?
                        html`
                            <dt-bell
...
function getNextBellIndex(dailyTimetable: DailyTimetable, now: Date) {
    const dailyTimetableDate = new Date(dailyTimetable.date);
    ...
    // Bells are sorted in ascending order, so we can just loop through them until we find one that hasn't happened yet
    for (let i = 0; i < bells.length; i++) {
        const startTime = timeToDate(bells[i].startTime, dailyTimetableDate);
        if (startTime.getTime() >= now.getTime()) return i;
    }
...
async function getNewDailyTimetable(currentDailyTimetable: DailyTimetable) : Promise<DailyTimetable | null> {
    ...
    for (const bell of generatedTimetable) {
