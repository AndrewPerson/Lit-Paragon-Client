...
`;class m extends e{static get styles(){return s}static get properties(){return{room:{type:String}}}constructor(){super(),this.room=""}render(){return i`
            <p>${this.room}</p>
        `}}class h extends e{static get styles(){return r}static get properties(){return{name:{type:String,converter:t=>t?.split(" ").filter((t=>isNaN(t)&&t.length>1)).join(" ")},room:{type:String}}}static highlighted="";static instances=[];static highlight(t){this.highlighted=t,this.instances.forEach((t=>t.update()))}constructor(){super(),h.instances.push(this),this.name="",this.room="",this.addEventListener("mouseover",(()=>{this.name&&this.room&&h.highlight(this.name)})),this.addEventListener("mouseleave",(()=>h.highlight("")))}render(){if(!this.name||!this.room)return i`<p></p>`;var t=h.highlighted==this.name,e=this.nextElementSibling,s=e?.nextElementSibling,r=e?.name==this.name||s?.name==this.name;return i`
            <div>
