...
`;customElements.define("bell-item",class extends ot{static get styles(){return[ct,_i]}static get properties(){return{name:{type:String},time:{type:String}}}constructor(){super(),this.name="",this.time="00:00"}render(){return B`
        ...
        `):B`<loading-element style="width: 80%"></loading-element>`:M}});const Ri=a`
    :host {
        position: relative;
...
`;class Fi extends ot{static get styles(){return[ct,Ui]}static get properties(){return{name:{type:String},room:{type:String}}}static highlighted="";static instances=[];static highlight(t){this.highlighted=t,this.instances.forEach((t=>t.requestUpdate()))}constructor(){super(),Fi.instances.push(this),this.name="",this.room=""}firstUpdated(){this.name&&(this.tabIndex=0,this.addEventListener("mouseover",(()=>Fi.highlight(this.name))),this.addEventListener("mouseleave",(()=>Fi.highlight(""))),this.addEventListener("focus",(()=>Fi.highlight(this.name))))}render(){if(Fi.highlighted==this.name&&this.name){var t=this.nextElementSibling,e=t?.nextElementSibling,i=t?.getAttribute("name")==this.name||e?.getAttribute("name")==this.name;return B`
        ...
        `}}),customElements.define("full-timetable",class extends ot{static get styles(){return[st,Yi]}static get properties(){return{data:{type:Object},day:{type:String}}}constructor(){super(),this.data={days:{}},this.day=""}render(){return this.hasAttribute("data")?this.data?(this.day=this.day.slice(0,3).toUpperCase()+" "+this.day.slice(-1),B`
        ...
        `):B`<loading-element style="width: 80%; height: 100%; margin: auto;"></loading-element>`:M}});const Xi=a`
